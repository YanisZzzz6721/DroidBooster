...
function CopyBtn({ text, label = "Copier" }: { text: string; label?: string }) {
  ...
  return (
    <button onClick={handle} style={{
      background: copied ? "var(--turquoise)" : "var(--bg-raised)",
      border: `1.5px solid ${copied ? "var(--turquoise)" : "var(--border-col)"}`,
      boxShadow: copied ? "none" : "2px 2px 0 var(--shadow-col)",
...
function CompanyCard({ group, onOpen, onRemove, onRegenerate }: {
  ...
  return (
    <div style={{
      ...
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = "6px 6px 0 var(--shadow-col)"; (e.currentTarget as HTMLElement).style.transform = "translate(-1px,-1px)" }}
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = "4px 4px 0 var(--shadow-col)"; (e.currentTarget as HTMLElement).style.transform = "translate(0,0)" }}
    >
      {/* Header entreprise */}
      <div
      ...
      >
        <div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 4 }}>
            <span style={{ fontSize: 20, fontWeight: 700, color: "var(--fg)" }}>{group.entreprise}</span>
            {group.poste && <span style={{ fontSize: 13, color: "var(--orange)", fontWeight: 600 }}>{group.poste}</span>}
          </div>
          {group.adresse && (
            <div style={{ fontSize: 12, color: "var(--fg-muted)", fontFamily: "'IBM Plex Mono', monospace", marginBottom: 8 }}>
              {group.adresse}
            </div>
          )}
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", color: "var(--fg-muted)" }}>
              {group.items.length} candidature{group.items.length > 1 ? "s" : ""}
            </span>
            {bestMatch > 0 && (
              <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, padding: "2px 8px", background: "var(--turquoise)", color: "#fff", border: "1.5px solid var(--border-col)" }}>
                Match {bestMatch}
              </span>
            )}
            {bestATS > 0 && (
              <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, padding: "2px 8px", background: scoreColor(bestATS), color: "#fff", border: "1.5px solid var(--border-col)" }}>
                ATS {bestATS}
              </span>
            )}
            {hasLettre && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: "2px 8px", border: "1.5px solid var(--border-col)" }}>Lettre</span>}
            {hasCV     && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: "2px 8px", border: "1.5px solid var(--border-col)" }}>CV</span>}
          </div>
        </div>
        <span style={{ fontSize: 20, color: "var(--fg-muted)", transition: "transform 0.2s", transform: expanded ? "rotate(90deg)" : "rotate(0deg)" }}>›</span>
      </div>

      {/* Détail candidatures */}
      {expanded && (
        <div style={{ borderTop: "1.5px solid var(--border-col)" }}>
          {group.items.map((c, i) => (
            <div key={c.id} style={{
              padding: "16px 24px",
              borderBottom: i < group.items.length - 1 ? "1px solid var(--border-col)" : "none",
              background: "var(--bg-raised)",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div>
                  <div style={{ fontSize: 12, fontFamily: "'IBM Plex Mono', monospace", color: "var(--fg-muted)", marginBottom: 4 }}>
                ...
                <button onClick={e => onRemove(c.id, e)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--fg-muted)", fontSize: 14 }}
                  onMouseEnter={e => (e.currentTarget.style.color = "#e05252")}
                  onMouseLeave={e => (e.currentTarget.style.color = "var(--fg-muted)")}>x</button>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                {/* Lettre */}
                {c.lettre_docx && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", border: "1px solid var(--border-col)", background: "var(--bg-raised)" }}>
                    ...
                    <button
                      ...
                      style={{ background: "var(--bg-raised)", border: "1.5px solid var(--border-col)", boxShadow: "2px 2px 0 var(--shadow-col)", color: "var(--fg)", padding: "4px 12px", fontSize: 11, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}
                    >
                      ✎ LibreOffice
                    </button>
                    <button
                      onClick={() => window.open(`${API}/download-lettre/${c.lettre_docx!.split("/").pop()}?t=${Date.now()}`, "_blank")}
                      style={{ background: "var(--orange)", border: "1.5px solid var(--border-col)", boxShadow: "2px 2px 0 var(--shadow-col)", color: "#fff", padding: "4px 12px", fontSize: 11, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}
                    >
                      ↓ Télécharger
                    </button>
                  </div>
                ...
                {c.cv_optimise_md && (
                  <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", border: "1px solid var(--border-col)", background: "var(--bg-raised)" }}>
                    ...
                    <button
                      ...
                      style={{ background: "var(--turquoise)", border: "1.5px solid var(--border-col)", boxShadow: "2px 2px 0 var(--shadow-col)", color: "#fff", padding: "4px 12px", fontSize: 11, cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontWeight: 600 }}
                    >
                      Telecharger
                    </button>
                  </div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  <button
                    onClick={() => onOpen(c.id)}
                    style={{ flex: 1, background: "var(--bg-raised)", border: "1.5px solid var(--border-col)", boxShadow: "3px 3px 0 var(--shadow-col)", color: "var(--fg)", padding: "8px", fontSize: 12, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600 }}
                  >
                  ...
                  {c.cv_optimise_md && (
                    <button
                      ...
                      style={{ background: "var(--orange)", border: "1.5px solid var(--border-col)", boxShadow: "3px 3px 0 var(--shadow-col)", color: "#fff", padding: "8px 16px", fontSize: 12, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600, whiteSpace: "nowrap" }}
                    >
                      Regenerer la lettre
                    </button>
                  )}
...
export default function History() {
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "50vh" }}>
      <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: "var(--fg-muted)" }}>Chargement...</span>
    </div>
  )
  ...
  if (view === "list") return (
    <div>
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 0, marginBottom: 24, border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)", width: "fit-content" }}>
        {[
          ...
          <button
            ...
            style={{
              ...
              background:  tab === t.key ? "var(--turquoise)" : "var(--bg-raised)",
              color:       tab === t.key ? "#fff" : "var(--fg)",
              border:      "none",
              borderRight: t.key === "chrono" ? "1.5px solid var(--border-col)" : "none",
              cursor:      "pointer",
              transition:  "all 0.1s",
            }}
          >
      ...
      {list.length === 0 ? (
        <div style={{ padding: "80px 32px", border: "1.5px dashed var(--border-col)", textAlign: "center", color: "var(--fg-muted)" }}>
          ...
          {list.map((c, i) => (
            <AnimatedCard key={c.id} delay={i * 40}>
              <div
                onClick={() => open(c.id)}
                style={{
                  border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)",
                  background: "var(--bg-surface)", padding: "20px 24px", cursor: "pointer",
                  transition: "transform 0.1s, box-shadow 0.1s",
                  display: "grid", gridTemplateColumns: "1fr auto", gap: 16, alignItems: "center",
                }}
                onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.transform = "translate(-2px,-2px)"; el.style.boxShadow = "6px 6px 0 var(--shadow-col)" }}
                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.transform = "translate(0,0)"; el.style.boxShadow = "4px 4px 0 var(--shadow-col)" }}
              >
                <div>
                  ...
                  <div style={{ display: "flex", gap: 16, marginBottom: 10 }}>
                    {c.adresse && <span style={{ fontSize: 12, color: "var(--fg-muted)", fontFamily: "'IBM Plex Mono', monospace" }}>{c.adresse}</span>}
                    <span style={{ fontSize: 12, color: "var(--fg-muted)", fontFamily: "'IBM Plex Mono', monospace" }}>{formatShort(c.created_at)}</span>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {c.match_score !== null && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, padding: "3px 10px", background: "var(--turquoise)", color: "#fff", border: "1.5px solid var(--border-col)" }}>Match {c.match_score}</span>}
                    {c.ats_score  !== null && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", fontWeight: 700, padding: "3px 10px", background: scoreColor(c.ats_score), color: "#fff", border: "1.5px solid var(--border-col)" }}>ATS {c.ats_score}</span>}
                    {c.lettre_docx    && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: "3px 10px", border: "1.5px solid var(--border-col)" }}>Lettre DOCX</span>}
                    {c.cv_optimise_md && <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: "3px 10px", border: "1.5px solid var(--border-col)" }}>CV optimise</span>}
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                  <button onClick={e => remove(c.id, e)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--fg-muted)", fontSize: 18 }}
                    onMouseEnter={e => (e.currentTarget.style.color = "#e05252")}
                    onMouseLeave={e => (e.currentTarget.style.color = "var(--fg-muted)")}>x</button>
                  <span style={{ fontSize: 20, color: "var(--fg-muted)" }}>›</span>
                </div>
              </div>
            </AnimatedCard>
          ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {regenResult && (
            <div style={{ border: "2px solid var(--turquoise)", boxShadow: "4px 4px 0 var(--turquoise)", background: "rgba(10,191,188,0.1)", padding: "20px 24px", animation: "fadeInUp 0.3s ease" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button onClick={() => setRegenResult(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--fg-muted)", fontSize: 16 }}>x</button>
                </div>
              </div>
              <div style={{ fontSize: 13, lineHeight: 1.8, whiteSpace: "pre-wrap", color: "var(--fg)", fontFamily: "'Space Grotesk', sans-serif" }}>
                {regenResult}
              </div>
            </div>
          )}
          ...
          {regenerating !== null && (
            <div style={{ padding: "20px", border: "1.5px solid var(--border-col)", textAlign: "center", fontFamily: "'IBM Plex Mono', monospace", fontSize: 13, color: "var(--fg-muted)" }}>
    ...
    <div>
      <button
        ...
        style={{
          ...
          background: "none", border: "1.5px solid var(--border-col)", boxShadow: "3px 3px 0 var(--shadow-col)",
        ...
        onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.transform = "translate(-1px,-1px)"; el.style.boxShadow = "4px 4px 0 var(--shadow-col)" }}
        onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.transform = "translate(0,0)"; el.style.boxShadow = "3px 3px 0 var(--shadow-col)" }}
      >
        &lt; Retour
      </button>

      {loadingDetail && (
        <div style={{ padding: "80px", textAlign: "center", border: "1.5px solid var(--border-col)" }}>
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: "var(--fg-muted)" }}>Chargement...</span>
        </div>
      )}
      ...
      {selected && !loadingDetail && (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          {/* RECAP */}
          <AnimatedCard>
            <div style={{ background: "var(--bg-raised)", border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)" }}>
          ...
          {selected.cv_optimise_md && (
            <AnimatedCard delay={80}>
              <div style={{ border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)" }}>
                ...
                <pre style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, lineHeight: 1.8, color: "var(--fg)", whiteSpace: "pre-wrap", padding: "24px 28px", margin: 0 }}>
                  {selected.cv_optimise_md}
                </pre>
              </div>
            </AnimatedCard>
            ...
            <AnimatedCard delay={120}>
              <div style={{ border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)" }}>
                ...
                {selected.ats_score !== null && (
                  <div style={{ padding: "14px 24px", borderBottom: "1px solid var(--border-col)", background: "var(--bg-raised)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
                    <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--fg-muted)" }}>Score ATS</span>
                ...
                <div style={{ padding: "28px", fontSize: 14, lineHeight: 2, color: "var(--fg)", whiteSpace: "pre-wrap" }}>
                  {selected.lettre_md}
                </div>
              </div>
            </AnimatedCard>
          ...
          {selected.offre_texte && (
            <AnimatedCard delay={160}>
              <div style={{ border: "1.5px solid var(--border-col)", boxShadow: "4px 4px 0 var(--shadow-col)" }}>
                ...
                <pre style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12, lineHeight: 1.7, color: "var(--fg)", whiteSpace: "pre-wrap", padding: "24px 28px", margin: 0 }}>
                  {selected.offre_texte}
                </pre>
              </div>
            </AnimatedCard>
